...
tags: ['Rendering', 'Frontend Architecture', 'Next.js', 'Performance', 'React']
---

## Introduction

"Where does my page get rendered?" sounds like it should have one answer. In 2026 it has at least five: Client-Side Rendering, Static Site Generation, Incremental Static Regeneration, classic Server-Side Rendering with hydration, and Server-Side Rendering with React Server Components. They produce roughly the same page for the user, but each one ships a different amount of JavaScript, builds HTML at a different moment, and fetches data in a different place — so the "right" choice depends entirely on the page.

Seven or eight years ago there was less to weigh. The strategy tended to follow the tool you picked: a content site reached for a static site generator, while anything with a login or a dashboard reached for `create-react-app` and got an empty `<div id="root" />`, a 200kb bundle, data fetched after mount, and a flash of "Loading…" before anything painted. Today a single framework can do all five, so the decision is yours to make on every route rather than something the toolchain settles for you.
...
- First Contentful Paint (FCP) is slow by definition. Every user pays the cost of the full bundle load plus the data round trip before they see anything meaningful.
...
Classic SSR is the model that dominated the React ecosystem from roughly 2018 through the App Router release. The initial request hits a server; later navigations are client-side, though in the Pages Router they still round-trip to the server to re-run `getServerSideProps` for the new page. The server runs the React tree to a string, attaches data as props, and ships the resulting HTML to the browser. The browser then downloads the JS bundle and *hydrates* the static markup — React mounts on top of the existing DOM, attaches event handlers, and reconciles.
...
- Fast FCP — the HTML arrives ready to paint.
...
One caveat on the RSC column: the numbers above describe the request-time, dynamic case, since that's the "SSR with RSC" model this post is about. RSC can also be rendered statically (the App Router does this by default when a route touches no request-time API), in which case its TTFB and freshness look like SSG or ISR — but the bundle-size win stays the same either way.
