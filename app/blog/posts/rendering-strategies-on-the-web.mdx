...
- Every request hits a server. You need Node infrastructure, and you need to think about caching SSR responses if traffic is high (see the [Cache-Control](/blog/caching-with-cache-control-header) post for that).

For long-running pages with multiple data sources, you can layer streaming on top — the [HTTP Streaming](/blog/http-streaming-benefits-and-drawbacks) post covers that pattern in detail.
...
If you want to go a layer deeper, two posts on this site cover the next level of detail: [HTTP streaming](/blog/http-streaming-benefits-and-drawbacks) for how SSR and RSC can deliver content progressively, and [Cache-Control](/blog/caching-with-cache-control-header) for how to make the request-time strategies almost as cheap to serve as the build-time ones.
